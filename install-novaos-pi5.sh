#!/usr/bin/env bash
# ================================================================
#  Nova OS v5  —  Raspberry Pi 5 Installer
#  Raspberry Pi OS Bookworm 64-bit (ARM64)
#
#  Place this file + NovaOS-Pi5.html in the same folder, then run:
...
echo "  ██║ ╚████║╚██████╔╝ ╚████╔╝ ██║  ██║    ╚██████╔╝███████║"
echo "  ╚═╝  ╚═══╝ ╚═════╝   ╚═══╝  ╚═╝  ╚═╝     ╚═════╝ ╚══════╝"
echo -e "  v5.0  ·  Raspberry Pi 5  ·  ARM64  ·  Native PyQt6 App${NC}"
echo ""
...
[[ "$CODENAME" == "bookworm" ]] && ok "Raspberry Pi OS Bookworm detected" \
  || warn "Expected Bookworm, got: $CODENAME — continuing"
...
# Download novaos.py from the same folder as this script,
# or from the embedded copy below if not found separately
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "$SCRIPT_DIR/novaos.py" ]]; then
  sudo cp "$SCRIPT_DIR/novaos.py" "$PYTHON_APP"
  ok "Copied novaos.py from $SCRIPT_DIR"
else
  warn "novaos.py not found next to installer — downloading from /opt/novaos/"
  # It should have been placed there by a prior step; if not, exit with instructions
  [[ -f "$PYTHON_APP" ]] || die "novaos.py not found. Place novaos.py in the same folder as this script."
fi
...
echo ""
echo -e "${CYAN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}${BOLD}  Nova OS v5 installed — native PyQt6 desktop app!${NC}"
echo -e "${CYAN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
